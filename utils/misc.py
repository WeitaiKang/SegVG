...
class CosineAnnealingRestartLR(_LRScheduler):
    ...
    def __init__(self,
        ...
        self.lr_rest_weight = [init_lr]*lr_drop + [init_lr*0.1]*(num_epoch-lr_drop)
    ...
    def get_lr(self):
        ...
        if current_weight != -1:
            res = self.lr_rest_weight[self.last_epoch:self.last_epoch+1] + [
                self.eta_min + current_weight * 0.5 * (base_lr - self.eta_min) *
                (1 + math.cos(math.pi * (
                    (self.last_epoch - nearest_restart) / current_period)))
                for base_lr in self.base_lrs[1:] # only modify NoneFusionPart
            ]
        else:
            res = [1e-5, 1e-6, 1e-6, 1e-6]
        return res
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None, epochs=None, epoch=0):
        i = 0
        if not header:
            header = ''
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                if epochs is not None:
                    eta_seconds = iter_time.global_avg * (len(iterable) - i + len(iterable) * (epochs-epoch-1)) # (len(iterable) - i)
                else:
...
def collate_fn(raw_batch):
    ...
    pos_region = torch.tensor(np.array(raw_batch[5]))
    batch = [img_data, text_data, bbox, pos_region]
    return tuple(batch)

...
class DETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor=None, src=None, mask=None, pos=None, layer_idx=0):
        ...
        if samples is not None:
            ...
            return self.input_proj(src).flatten(2).permute(2, 0, 1), mask.flatten(1), pos[-1].flatten(2).permute(2, 0, 1)
        
        if src is not None:
            if self.transformer is not None:
                mask, out = self.transformer(src, mask, pos, query_embed=None, layer_idx=layer_idx)
            else:

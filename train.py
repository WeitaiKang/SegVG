...
def get_args_parser():
    ...
    parser.add_argument('--tensorboard',
                        default='/data/kangweitai/VG/tensorboard/SegVG/',
...
def main(args):
    ...
    visu_pNum = [p.numel() for n, p in model_without_ddp.named_parameters() if (("visumodel" in n) and p.requires_grad)]
    text_pNum = [p.numel() for n, p in model_without_ddp.named_parameters() if (("textmodel" in n) and p.requires_grad)]
    rest_pNum = [p.numel() for n, p in model_without_ddp.named_parameters() if (("visumodel" not in n) and ("textmodel" not in n) and p.requires_grad)]
    print('number of visu_pNum:', sum(visu_pNum) / 1e6, 'M')
    print('number of text_pNum:', sum(text_pNum) / 1e6, 'M')
    print('number of rest_pNum:', sum(rest_pNum) / 1e6, 'M')
    ...
    if args.lr_scheduler == 'poly':
        ...
        lr_scheduler = utils.CosineAnnealingRestartLR(optimizer, periods=[10+20] + [30] + [30] + [30],
                                                                   restart_weights=[0] + [0.5] * 2 + [-1],
                                                                   eta_min=args.lr_bert * 0.1,
                                                                   init_lr=args.lr, lr_drop=args.epochs,
    ...
    if args.resume:
        ...
        if 'best_accu' in checkpoint.keys():
    ...
    if args.start_epoch < args.freeze_epochs:
        for n, p in model_without_ddp.named_parameters():
            if ("visumodel" in n) or ("textmodel" in n): 
    ...
    print(f'--freeze_epochs {args.freeze_epochs} --lr_drop {args.lr_drop} --epochs {args.epochs}  --start_epoch {args.start_epoch}')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            args, model, data_loader_train, optimizer, device, epoch, args.clip_max_norm, writer=writer, is_main_process=is_main_process, epochs = args.epochs
        )

        if (epoch + 1) == args.freeze_epochs:
            for n, p in model_without_ddp.named_parameters():
                if (("visumodel" in n) or ("textmodel" in n)) and ('pooler' not in n): 
        ...
        val_stats = validate(args, model, data_loader_val, device, writer, epoch, is_main_process)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('SegVG training script', parents=[get_args_parser()])

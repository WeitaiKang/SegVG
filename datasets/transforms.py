...
class RandomSizeCrop(object):
    ...
    def __call__(self, input_dict):
        ...
        for i in range(self.max_try):
            ...
            if self.check_iou(cropped_box, orig_box):
                img, box = self.crop(img, orig_box, region)
                input_dict['img'] = img
                input_dict['box'] = box
                return input_dict
        return input_dict
    ...
    def check_iou(self, cropped_box, orig_box):
        iou = self.box_iou(cropped_box.view(-1, 4), orig_box.view(-1, 4))[0]
        return (iou >= 0.5).all()
    ...
    def crop(self, image, box, region):
        cropped_image = F.crop(image, *region)
...
class NormalizeAndPad(object):
    ...
    def __call__(self, input_dict):
        ...
        out_mask = torch.zeros((self.size, self.size)).int()
        ...
        if 'box' in input_dict.keys():
            ...
            # positive region
            pos_region = torch.zeros((self.size, self.size)).int()
            pos_region[int(box[1]): int(box[3]), int(box[0]): int(box[2])] = 1.
            input_dict['pos_region'] = pos_region
